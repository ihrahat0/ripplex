...
import { useRealTimeData } from '../../assets/fake-data/data-coin';
...
function Services02(props) {
    ...
    const { data: coinData, loading } = useRealTimeData();
    const [selectedCoin, setSelectedCoin] = useState(null);

    useEffect(() => {
        if (coinData.length > 0) {
            setSelectedCoin(coinData[0]); // Set Bitcoin as default
        }
    }, [coinData]);

    const [dataBlock] = useState(
        {
            heading: 'Why choose Ripple Exchange',
            desc : 'Ripple Exchange has a variety of features that make it the best place to start trading'
        }
    )
        ...
        <div className="container">
            ...
            <div className="col-xl-6 col-md-12">
...

                        {
                            dataIcon.map((data,idx) => (
                                <li className="icon" key={idx}>
                                    <Link to="#"
                                    ><img src={data.icon} alt="Ripple Exchange"
                                    /></Link>
                                </li>
                            ))
                        }
                ...
                <div className="crypto-box">
                    <img className="arrow" src={arrow} alt="Ripple Exchange" />
                    <div className="left">
                    <img src={selectedCoin.icon} alt={selectedCoin.name} />
                    <div>
                        <h6>{selectedCoin.uint}</h6>
                        <p>{selectedCoin.sale}</p>
                    </div>
                    </div>

                    <div className="right">
                    <h6 className="price">{selectedCoin.price}</h6>
                    <div className={`chart ${selectedCoin.class}`}>
                        {/* Add a mini chart component here if needed */}
                    </div>
                    </div>
                </div>
